...
export function GraphPage() {
  ...
  const updateGraphData = (orgRoamGraphData: OrgRoamGraphReponse) => {
    ...
    // Generate heading hierarchy links using two strategies:
    // 1. OLP-based: use olp (outline path) to find parent by matching the last
    //    olp entry against node titles in the same file (most accurate)
    // 2. Position-based fallback: walk backwards by position to find the nearest
    //    preceding node with a smaller level
    const headingLinks: OrgRoamLink[] = Object.keys(nodesByFile).flatMap((file) => {
      ...
      const headingNodes = nodesInFile
      ...
      // Build a title→node index for olp matching (strip TODO keywords, priorities, statistics)
      const titleIndex: { [title: string]: OrgRoamNode[] } = {}
      for (const node of nodesInFile) {
        const cleanTitle = (node.title as string)?.replace(/ *\[\d*\/\d*\] */g, '').trim()
        if (cleanTitle) {
          if (!titleIndex[cleanTitle]) titleIndex[cleanTitle] = []
          titleIndex[cleanTitle].push(node)
        }
      }

      return headingNodes.map((headingNode, idx) => {
        const olp = headingNode.olp as string[] | null
        let parentNode: OrgRoamNode | undefined

        // Strategy 1: OLP-based — last olp entry is the direct parent's title
        if (olp && olp.length > 0) {
          const parentTitle = olp[olp.length - 1]
          const candidates = titleIndex[parentTitle] ?? []
          // Among candidates, pick the one preceding this node with a smaller level
          parentNode = candidates
            .filter((c) => c.level < headingNode.level && c.pos < headingNode.pos)
        ...
        if (!parentNode) {
          for (let i = idx - 1; i >= 0; i--) {
            if (headingNodes[i].level < headingNode.level) {
              parentNode = headingNodes[i]
              break
            }
          }
        }
  ...
  return (
    <VariablesContext.Provider value={{ ...emacsVariables }}>
      <Box
        ...
        overflow="hidden"
      >
        {/* Layer 0: Graph canvas — fills entire viewport, receives all pointer events */}
        ...
        {/* Layer 1: Tweaks panel (top-left settings) */}
        <Tweaks
          {...{
            physics,
            setPhysics,
        ...
        {/* Layer 2: Header bar (top-right buttons) — pointer-events only on buttons */}
        <Flex
          position="absolute"
          top={0}
          right={0}
          ...
          pr={2}
        >
          <Flex height="100%" flexDirection="row" pointerEvents="auto">
        ...
        {/* Layer 3: Sidebar (right side) */}
        <Box position="absolute" top={0} right={0} zIndex={4} height="100vh" pointerEvents="none">
        ...
        {contextMenu.isOpen && (
          <div ref={contextMenuRef} style={{ position: 'absolute', inset: 0, zIndex: 5, pointerEvents: 'none' }}>
            <Box pointerEvents="auto">
              <ContextMenu
...
export const Graph = function (props: GraphProps) {
  ...
  const handleClick = (click: string, node: OrgRoamNode, event: any) => {
    switch (click) {
      case mouse.preview: {
        setPreviewNode(node)
        // Also open node in Emacs when clicking
        if (webSocket && node?.id) {
  ...
  const graphCommonProps: ComponentPropsWithoutRef<typeof TForceGraph2D> = {
    ...
    linkColor: (link) => {
      ...
      // Semantic relation links: use color from Emacs metadata
      if (roamLink.color && !['id', 'ref', 'cite', 'heading', 'parent', 'bad'].includes(roamLink.type)) {
        return roamLink.color
      }
