...
* 📦 Git Submodule Package Management
...
├── core/           # Core libraries (dash, s.el, f.el, compat, lazy-load)
├── completion/     # Completion system (vertico, consult, marginalia, orderless, embark)
├── editor/         # Editor features (avy, ace-window, vundo, markmacro, fingertip)
├── git/            # Git integration (magit, with-editor, jujutsu)
├── input/          # Input methods (emacs-rime, posframe)
├── lsp/            # LSP and language support (lsp-bridge, rust-mode, lua-mode, etc.)
├── org/            # Org-mode extensions (org-modern, ox-hugo, org-download)
├── search/         # Search tools (blink-search, color-rg, consult-todo)
├── snippets/       # Snippet libraries (yasnippet, yasnippet-snippets)
├── ui/             # UI components (awesome-tray, themes/, nerd-icons)
├── utils/          # Utility packages (helpful, json-mode, hydra)
└── docs/           # Documentation tools (markdown-ts-mode, flymake-vale)
#+end_src
...
| Key     | Description                    |
|---------+--------------------------------|
| C-c s l | Search line (consult-line)     |
| C-c s r | Search with ripgrep (consult)  |
| C-c s i | Search imenu (consult)         |
| C-c s o | Search outline (consult)       |
| C-c s g | Search content (color-rg)      |
| C-c s G | Search project (color-rg)      |
| C-c s s | Search symbol (color-rg)       |
| C-c s b | Blink search                   |
| C-c s d | Search current directory       |
| C-c s p | Search project root            |
...
| C-c b m | Switch bookmark                |
| C-c b r | Open recent file               |
| C-c f f | Find file                      |
