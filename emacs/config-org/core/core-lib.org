...
(defvar mcg-extension-cache-version 2
...
(defun mcg--extension-name-string (name)
  "Return canonical string representation for extension NAME."
...
(defun mcg--find-el-files-in-dir (dir)
  "Check if DIR contains loadable .el files.
Package metadata such as .dir-locals.el is ignored."
    ...
    (when (and (> depth 0) (file-directory-p dir))
      (dolist (entry (directory-files dir t "^[^.]"))
        (when (and (file-directory-p entry)
                   (not (mcg--extension-dir-excluded-p entry)))
          (let* ((name (file-name-nondirectory entry))
                 (lisp-dir (expand-file-name "lisp" entry)))
            ;; Prefer conventional package lisp/ directories over repository roots.
            (cond
             ((mcg--find-el-files-in-dir lisp-dir)
              (push (cons name lisp-dir) results))
             ((mcg--find-el-files-in-dir entry)
              (push (cons name entry) results))
             (t
              (setq results
                    (append results
                            (mcg--discover-extensions-recursive
                             entry (1- depth))))))))))
    results))
#+end_src
...
Returns the full directory path or nil if not found."
  ...
  (let ((name-str (mcg--extension-name-string name)))
    (unless (mcg-cache-valid-p)
      (mcg-cache-clear)
      (mcg-cache-update-signature))

    ;; First check cache
    (let ((cached (mcg-cache-lookup name-str)))
      (if cached
          cached
        ;; Not in cache, search for it
        (let ((extensions (mcg-list-extensions)))
          (cdr (assoc name-str extensions)))))))

(defun mcg--extension-check-path (path)
  "Return extension PATH or its lisp subdir when valid."
  (when (file-directory-p path)
    (or (let ((lisp-path (expand-file-name "lisp" path)))
          (and (mcg--find-el-files-in-dir lisp-path) lisp-path))
        (and (mcg--find-el-files-in-dir path) path))))

(defun mcg-extension-direct-path (name)
  "Try to find extension NAME by direct path lookup.
Checks common locations like extensions/NAME, extensions/git/NAME, extensions/*/*/NAME, etc.
Returns the path if found, nil otherwise."
  ...
  (let ((name-str (mcg--extension-name-string name))
    ...
    (when (file-directory-p mcg-extensions-dir)
      ...
      (setq result (mcg--extension-check-path (expand-file-name name-str mcg-extensions-dir)))

      ;; If try subdirectory pattern not found yet,: extensions/*/name
      (unless result
        (dolist (subdir (directory-files mcg-extensions-dir t "^[^.]"))
          (unless result
            (when (and (file-directory-p subdir)
                       (not (mcg--extension-dir-excluded-p subdir)))
              (let ((ext-path (expand-file-name name-str subdir)))
                (setq result (mcg--extension-check-path ext-path)))))))

      ;; If still not found, try two-level pattern: extensions/*/*/name
      (unless result
        (dolist (subdir1 (directory-files mcg-extensions-dir t "^[^.]"))
          (unless result
            (when (and (file-directory-p subdir1)
                       (not (mcg--extension-dir-excluded-p subdir1)))
              (dolist (subdir2 (directory-files subdir1 t "^[^.]"))
                (unless result
                  (when (and (file-directory-p subdir2)
                             (not (mcg--extension-dir-excluded-p subdir2)))
                    (let ((ext-path (expand-file-name name-str subdir2)))
                      (setq result (mcg--extension-check-path ext-path)))))))))))
...
if the extension is not found."
  ...
  (let* ((path (or (mcg-extension-path name)
                   (mcg-extension-direct-path name)))
         (path (and path
                    (or (mcg--extension-check-path path)
                        path))))
    (if path
        (progn
...
模块定义模板宏，用于简化模块配置文件的编写。

#+begin_src emacs-lisp
(defmacro mcg-defmodule (name &rest body)
  "Define an MCG module with standard structure.

NAME is the module name (symbol), used for logging.
BODY contains the module configuration code.

This macro provides:
- Standard error handling
- Logging of module setup
- Feature provision

Example usage:
  (mcg-defmodule my-module
    :config
    (setq my-var t)
    :bind
    (\"C-c m\" . my-command))
...
Keywords:
  :config  - Configuration code to execute
  :bind    - Keybindings as alist
  :hook    - Mode hooks as alist (mode . function)
  :after   - Features to load after (symbols or list)
  :custom  - Custom variables as alist (var . value)"
  (declare (indent 1))
  (let ((config-code nil)
        (bindings nil)
        (hooks nil)
    ...
    (while body
      ...
      (cond
       ((eq key :config)
        (setq config-code (cadr body))
        (setq body (cddr body)))
       ((eq key :bind)
        (setq bindings (cadr body))
        (setq body (cddr body)))
       ((eq key :hook)
        (setq hooks (cadr body))
        (setq body (cddr body)))
       ((eq key :after)
        (setq after-features (cadr body))
        (setq body (cddr body)))
       ((eq key :custom)
        (setq custom-vars (cadr body))
        (setq body (cddr body)))
       (t
        (setq body (cdr body)))))
    `(progn
       ...
       ,(if config-code
            `(condition-case err
                 (progn
                   ,config-code
                   (mcg-log "Module '%s' configured" ',name))
               (error
                (mcg-log-error "Module '%s' configuration failed: %s"
                               ',name (error-message-string err))))
          `(mcg-log "Module '%s' configured" ',name))
       ;; Setup keybindings
       ,@(mapcar (lambda (b)
                   `(mcg-bind-key ,(car b) ,(cdr b) ,(format "%s" name)))
                 bindings)
       ;; Load after features
       ,@(mapcar (lambda (f)
                   `(with-eval-after-load ',f
                      (mcg-log "Module '%s' post-load hooks executed" ',name)))
                 after-features))))
#+end_src

*** Simple Module Macro

更简单的模块定义宏，适用于不需要复杂配置的场景。

#+begin_src emacs-lisp
(defmacro mcg-simple-module (name setup-fn &optional keybindings)
  "Define a simple MCG module with setup function and optional keybindings.

NAME is the module name (string) for logging.
SETUP-FN is the setup function to call.
KEYBINDINGS is an optional alist of (key . command) pairs.
