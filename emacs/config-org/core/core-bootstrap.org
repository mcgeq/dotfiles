...
#+begin_src emacs-lisp
(defgroup mcg nil
  "MCG Emacs configuration framework."
  :group 'convenience
  :prefix "mcg-")

(defcustom mcg-gc-cons-threshold-default (* 16 1024 1024)
  "Default GC threshold after startup (16MB).
This is restored after Emacs finishes starting up."
  :type 'integer
  :group 'mcg)

(defcustom mcg-gc-cons-percentage-default 0.1
  "Default GC percentage after startup.
This is restored after Emacs finishes starting up."
  :type 'float
  :group 'mcg)

(defcustom mcg-gc-cons-threshold-startup (* 256 1024 1024)
...
(defcustom mcg--log-max-messages 100
...
#+begin_src emacs-lisp
(defcustom mcg-extension-load-path-excluded-dirs
  '("test" "tests" "testing"
    "example" "examples"
    "demo" "demos"
    "docs" "doc"
    ...
    "workflow" "workflows"
    "scripts" "ci")
  "Directory names excluded when scanning extensions for `load-path'."
  :type '(repeat string)
  :group 'mcg)

(defun mcg--extension-dir-excluded-p (dir)
  "Return non-nil when DIR should be excluded from extension scanning."
  (let ((name (file-name-nondirectory (directory-file-name dir))))
    (or (string-prefix-p "." name)
        (member name mcg-extension-load-path-excluded-dirs))))

(defun mcg--add-to-load-path-front (dir)
  "Add DIR to `load-path' when it is an existing directory."
  (when (file-directory-p dir)
    (add-to-list 'load-path (file-name-as-directory dir))))

(defun mcg--directory-has-el-files-p (dir)
  "Return non-nil when DIR contains Emacs Lisp source files."
...
Adds core, modules, and extensions directories to load-path."
  ...
  (mcg--add-to-load-path-front mcg-private-dir)
...
Only adds directories that contain .el files."
  (when (and (file-directory-p dir)
             (not (mcg--extension-dir-excluded-p dir)))
    (dolist (entry (directory-files dir t "^[^.]"))
      (when (file-directory-p entry)
        (unless (mcg--extension-dir-excluded-p entry)
          ;; Check if directory contains .el files
          (if (mcg--directory-has-el-files-p entry)
              (mcg--add-to-load-path-front entry)
...
Sets high GC threshold during startup and restores it after."
  ...
  ;; Restore file-name handlers from early-init
...
(defun mcg-restore-startup-optimizations ()
  "Restore startup optimizations changed in early-init.
Restores `file-name-handler-alist' when early-init saved its original value."
  (when (boundp 'mcg--file-name-handler-alist-original)
    (setq file-name-handler-alist mcg--file-name-handler-alist-original)
    (mcg-log "Restored file-name-handler-alist")))
#+end_src
