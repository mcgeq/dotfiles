...
- list of hooks (for multiple mode support)")

(defvar mcg-module-triggers
  '(;; Language module triggers (using tree-sitter mode hooks)
    ((:lang . rust) . (rust-mode-hook rust-ts-mode-hook))
    ((:lang . python) . (python-mode-hook python-ts-mode-hook))
    ((:lang . typescript) . (typescript-mode-hook typescript-ts-mode-hook tsx-ts-mode-hook js-mode-hook js-ts-mode-hook))
    ((:lang . web) . ("\\.vue\\'" . web-mode))
    ((:lang . lua) . (lua-mode-hook lua-ts-mode-hook))
    ((:lang . zig) . (zig-mode-hook zig-ts-mode-hook))
    ((:lang . cpp) . (c-mode-hook c++-mode-hook c-ts-mode-hook c++-ts-mode-hook))
    ((:lang . clojure) . (clojure-mode-hook clojurescript-mode-hook clojurec-mode-hook))
    ;; Writing module triggers
    ((:writing . markdown) . (markdown-mode-hook gfm-mode-hook))
    ((:writing . vale) . (markdown-mode-hook)))
  "Module trigger mapping.

Format is alist of ((CATEGORY . MODULE) . TRIGGER).
TRIGGER can be:
- (PATTERN . MODE) - auto-mode-alist entry
- HOOK-SYMBOL - single hook
- (HOOK1 HOOK2 ...) - list of hooks for multiple mode support")
...
- (HOOK1 HOOK2 ...) - list of hooks"
  (let* ((cat-key (if (keywordp category) category
    ...
    (when trig
      (cond
       ;; List of hooks (for multiple mode support)
       ((and (listp trig) (symbolp (car trig)) (string-suffix-p "-hook" (symbol-name (car trig))))
        (let ((category cat-key)
              (module mod-sym))
          (dolist (hook trig)
            ;; Use depth -90 to run before lsp-bridge (which uses default depth 0)
            (add-hook hook
                      (lambda ()
                        (mcg--trigger-deferred-load category module))
                      -90))))
       
       ;; auto-mode-alist trigger (PATTERN . MODE)
