...
Structure: (((CATEGORY . MODULE) . ERROR-MESSAGE) ...)")
...
(defvar mcg-idle-load-delay 2.0
    ...
    (if (member mod-key mcg-loaded-modules)
      ...
      (if (not (file-exists-p path))
          (let ((err-msg (format "Module file not found: %s" path)))
            (push (cons mod-key err-msg) mcg-module-load-errors)
...
Errors are recorded with full trace in `mcg-module-error-traces' and `mcg-module-load-errors'."
  (let ((mod-key (cons category (if (symbolp module) module (intern module))))
    ...
    (if (member mod-key mcg-loaded-modules)
      ...
      (if (not (file-exists-p path))
          (let* ((err-msg (format "Module file not found: %s" path))
                 (error-info (list :category category
                                   :module mod-sym
                                   :error err-msg
                                   :message err-msg
                                   :path path
                                   ...
                                   :backtrace nil)))
            ;; Store detailed error info in both places
            (push (cons mod-key error-info) mcg-module-load-errors)
            (push (cons mod-key error-info) mcg-module-error-traces)
            (mcg-log-warning "%s" err-msg)
            nil)
        ;; Load the module with error handling
        (condition-case err
    ...
    (when mcg-module-load-errors
      ...
      (dolist (err-entry mcg-module-load-errors)
        (let ((mod-key (car err-entry))
              (err-msg (cdr err-entry)))
          (insert (format "    ✗ %s/%s\n      %s\n"
                          (car mod-key) (cdr mod-key)
                          (if (listp err-msg)
                              (plist-get err-msg :error)
                            err-msg)))))
      (insert "\n"))
