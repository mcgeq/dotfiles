...
(defvar mcg-mode-extension-paths
  '((web-mode . "lsp/web-mode")
    (rust-mode . "lsp/rust-mode")
    (lua-mode . "lsp/lua-mode")
    (zig-mode . "lsp/zig"))
  "Mode 对应的扩展路径映射。

用于在设置 autoload 时找到正确的扩展路径。")
...
如果 MODE 对应的扩展路径存在，设置 autoload。"
  (unless (fboundp mode)
    (let* ((ext-path (cdr (assq mode mcg-mode-extension-paths)))
           (full-path (when ext-path
                        (expand-file-name ext-path mcg-extensions-dir))))
