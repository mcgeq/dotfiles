...
(defun mcg--module-symbol (module)
  "Return MODULE as a symbol."
  (if (symbolp module) module (intern module)))

(defun mcg--module-key (category module)
  "Return normalized module key for CATEGORY and MODULE."
  (cons category (mcg--module-symbol module)))
...
Returns the path to init-MODULE.el file."
  (let ((cat-dir (mcg-category-dir category))
        (mod-name (symbol-name (mcg--module-symbol module))))
...
Returns non-nil if the module is in mcg-modules-alist."
  (let ((cat-modules (cdr (assq category mcg-modules-alist))))
    (memq (mcg--module-symbol module) cat-modules)))
...
Returns non-nil if the module is in mcg-disabled-modules."
  (let ((cat-disabled (cdr (assq category mcg-disabled-modules))))
    (memq (mcg--module-symbol module) cat-disabled)))

(defun mcg-module-loaded-p (category module)
  "Check if MODULE in CATEGORY has been loaded."
  (member (mcg--module-key category module) mcg-loaded-modules))
...
(defun mcg--clear-module-error (mod-key)
  "Clear stale error records for MOD-KEY."
  (setq mcg-module-load-errors
        (assoc-delete-all mod-key mcg-module-load-errors #'equal))
  (setq mcg-module-error-traces
        (assoc-delete-all mod-key mcg-module-error-traces #'equal)))

(defun mcg--load-module-internal (category module &optional include-backtrace)
  "Load MODULE from CATEGORY.
When INCLUDE-BACKTRACE is non-nil, record a backtrace on failure.
Returns t if successful, nil if failed."
  (let* ((mod-sym (mcg--module-symbol module))
         (mod-key (mcg--module-key category module))
         (path (mcg-module-path category module)))
    (cond
     ((member mod-key mcg-loaded-modules)
      (mcg-log "Module %s/%s already loaded" category mod-sym)
      t)
     ((not (file-exists-p path))
      (let* ((err-msg (format "Module file not found: %s" path))
             (error-info (mcg--make-module-error-info category mod-sym path err-msg)))
        (mcg--record-module-error mod-key error-info)
        (mcg-log-warning "%s" err-msg)
        nil))
     (t
      (condition-case err
          (mcg-profile (format "%s/%s" category mod-sym)
            (load path nil t)
            (push mod-key mcg-loaded-modules)
            (mcg--clear-module-error mod-key)
            (run-hook-with-args 'mcg-module-load-hook category mod-sym)
            (mcg-log "Loaded module %s/%s" category mod-sym)
            t)
        (error
         (let* ((err-msg (error-message-string err))
                (error-info
                 (mcg--make-module-error-info
                  category mod-sym path err-msg
                  (when include-backtrace
                    (with-output-to-string (backtrace))))))
           (mcg--record-module-error mod-key error-info)
           (mcg-log-error "Failed to load %s/%s: %s" category mod-sym err-msg)
           nil)))))))
#+end_src
...
Errors are recorded in `mcg-module-load-errors'."
  (mcg--load-module-internal category module))
#+end_src
...
Errors are recorded with full trace in `mcg-module-error-traces' and `mcg-module-load-errors'."
  (mcg--load-module-internal category module t))
#+end_src
...
Returns the error plist or nil if no error recorded."
  (let ((mod-key (mcg--module-key category module)))
...
TRIGGER is (PATTERN . MODE) or looked up from `mcg-module-triggers'."
  (let* ((mod-key (mcg--module-key category module))
    ...
    (when (and trigger
               ...
               (not (assoc mod-key mcg-deferred-modules)))
...
MOD-KEY is (CATEGORY . MODULE)."
  (let ((category (car mod-key))
    ...
    (setq mcg-deferred-modules
          (assoc-delete-all mod-key mcg-deferred-modules #'equal))
    ;; Load the module
    (mcg-load-module category module)))

(defun mcg-setup-deferred-loading (&optional categories)
  "Set up deferred loading for trigger-backed modules.
When CATEGORIES is non-nil, only modules from those categories are deferred."
  (dolist (mod-key mcg-module-load-order)
    (let* ((category (car mod-key))
           (module (cdr mod-key))
