...
;; 确保常用缩进变量已定义（避免 lsp-bridge 等包在 json-mode 等模式下报错）
(unless (boundp 'js-indent-level)
  (defvar js-indent-level 2 "JavaScript 缩进级别。"))
#+end_src
...
(defun mcg-editor--adjust-languages-indent (n)
  ...
  ;; JavaScript (only if the variable exists)
  ...
  (when (boundp 'coffee-tab-width)
    (setq-local coffee-tab-width n))
  ;; Web mode
  (when (boundp 'web-mode-attr-indent-offset)
    (setq-local web-mode-attr-indent-offset n))
  (when (boundp 'web-mode-attr-value-indent-offset)
  ...
  (when (boundp 'web-mode-sql-indent-offset)
...
** Smart Kill Configuration

为编程和 Markdown 模式提供结构优先的 `C-k`，并在 Org 模式保留默认 `C-k`。
...
(defun mcg/kill-dwim ()
  "Kill region or next sexp, fallback to line.
If region is active, kill region.
Otherwise try `kill-sexp' first; if it fails, kill line.

In Org mode, keep default `C-k` behavior and use `C-M-k` for sexp kill."
  (interactive)
  (cond
   ((use-region-p)
...
(defvar mcg-smart-kill-mode-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "C-k") #'mcg/kill-dwim)
    (define-key map [remap kill-line] #'mcg/kill-dwim)
    (define-key map [remap kill-visual-line] #'mcg/kill-dwim)
    map)
  "Keymap for `mcg-smart-kill-mode'.")

(defvar mcg-smart-kill-emulation-map-alist
  `((mcg-smart-kill-mode . ,mcg-smart-kill-mode-map))
  "Emulation map alist for `mcg-smart-kill-mode'.")

(add-to-list 'emulation-mode-map-alists 'mcg-smart-kill-emulation-map-alist)

(define-minor-mode mcg-smart-kill-mode
  "Minor mode for structural-first `C-k'."
  :init-value nil
  :lighter nil
  :keymap mcg-smart-kill-mode-map)

(defun mcg-editor--setup-smart-kill-keybindings ()
  "Setup smart kill keybindings for different major modes."
  ;; Programming and Markdown modes: use smart structural kill via minor mode.
  (add-hook 'prog-mode-hook #'mcg-smart-kill-mode)
  (add-hook 'markdown-mode-hook #'mcg-smart-kill-mode)

  ;; Org: keep default C-k behavior, ensure C-M-k is structural kill.
  (add-hook 'org-mode-hook
            (lambda ()
              (local-set-key (kbd "C-M-k") #'kill-sexp)))

  (mcg-log "Configured smart kill keybindings"))
#+end_src
...
(mcg-editor--setup-smart-kill-keybindings)
#+end_src
