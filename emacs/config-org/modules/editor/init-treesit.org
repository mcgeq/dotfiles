...
本模块负责 MCG Emacs 的 Tree-sitter 配置，包括：
- treesit 语言源配置
- major-mode 重映射
- treesit-auto 自动安装
- 文件类型关联
...
;; The external typescript-mode package has a different mode hierarchy than
;; the built-in typescript-ts-mode, causing "Inconsistent hierarchy" errors.
;; We prevent it from being loaded by adding it to load-path exclusions.
(with-eval-after-load 'typescript-mode
  ;; If typescript-mode gets loaded anyway, warn the user
  (message "Warning: typescript-mode package loaded. This may cause hierarchy conflicts with typescript-ts-mode."))

;; Remove typescript-mode from auto-mode-alist if it gets added
...
;; Run after init to ensure our settings take precedence
...
(defvar mcg-treesit-major-mode-remap
  '((c-mode . c-ts-mode)
    ...
    ;; Note: typescript-mode is not remapped because it's an external package
    ;; with different mode hierarchy. Use typescript-ts-mode directly via auto-mode-alist.
...
(defun mcg-treesit-setup-auto-mode ()
  ...
  ;; Add entries in reverse order so higher priority ones end up first
  (dolist (pair mcg-treesit-auto-mode-alist)
    (add-to-list 'auto-mode-alist pair))
  (add-to-list 'interpreter-mode-alist '("coffee" . coffee-mode))
  
  ;; Ensure TypeScript/TSX use tree-sitter modes (highest priority)
  ;; These must be at the front of auto-mode-alist to override any other settings
  (setq auto-mode-alist
        (append '(("\\.ts\\'" . typescript-ts-mode)
                ...
                (seq-remove (lambda (pair)
                              (and (stringp (car pair))
                                   (string-match-p "\\.\\(ts\\|tsx\\|mts\\|cts\\)\\\\'" (car pair))))
                            auto-mode-alist)))
...
Note: treesit-auto is disabled for TypeScript/TSX to avoid mode hierarchy conflicts.
TypeScript files use typescript-ts-mode directly via auto-mode-alist."
  (when (mcg-require-extension "treesit-auto" t)
    ;; Configure treesit-auto BEFORE loading it
    (setq treesit-auto-install mcg-treesit-auto-install)
    
    ;; Exclude TypeScript modes from treesit-auto to prevent hierarchy conflicts
    ;; typescript-mode (external package) has different inheritance than typescript-ts-mode
    (setq treesit-auto-langs
          (seq-difference '(bash c cpp css dockerfile go html java javascript
                            json lua make markdown python ruby rust toml yaml)
                          '(typescript tsx)))
    
    (require 'treesit-auto nil t)
    (when (featurep 'treesit-auto)
      ;; Remove TypeScript from fallback alist
      (when (boundp 'treesit-auto-fallback-alist)
        (setq treesit-auto-fallback-alist
              (seq-remove (lambda (entry)
                            (memq (car entry) '(typescript-ts-mode tsx-ts-mode js-ts-mode)))
                          treesit-auto-fallback-alist)))
      ;; Remove TypeScript from recipe list
      (when (boundp 'treesit-auto-recipe-list)
        (setq treesit-auto-recipe-list
              (seq-remove (lambda (recipe)
                            (and (treesit-auto-recipe-p recipe)
                                 (memq (treesit-auto-recipe-ts-mode recipe)
                                       '(typescript-ts-mode tsx-ts-mode))))
                          treesit-auto-recipe-list)))
      (global-treesit-auto-mode 1)
      (mcg-log "Enabled treesit-auto-mode (TypeScript excluded)"))))
#+end_src
...
;; NOTE: treesit-auto is DISABLED to avoid TypeScript mode hierarchy conflicts
;; The typescript-mode package defines a mode with different inheritance than
;; typescript-ts-mode, causing "Inconsistent hierarchy" errors.
;; TypeScript files use typescript-ts-mode directly via auto-mode-alist.
;; (when (mcg-extension-exists-p "treesit-auto")
