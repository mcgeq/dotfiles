...
(defun mcg--setup-search-keybindings ()
  ...
  (when (featurep 'blink-search)
    (global-set-key (kbd "C-c f b") 'blink-search)
    (global-set-key (kbd "C-c f d") 'mcg/blink-search-grep-current-dir)
    (global-set-key (kbd "C-c f p") 'mcg/blink-search-project-root))
