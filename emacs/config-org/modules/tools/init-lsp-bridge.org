...
#+begin_src emacs-lisp
  ...
  (defun mcg-lsp-format-and-save-after-format (_filename _edits)
    ...
    (if (not (file-directory-p lsp-bridge-path))
      ...
      (if (require 'lsp-bridge nil t)
          (progn
            ...
            ;; Optional snippet backend - only enable if yasnippet is already loaded
            ;; (init-yasnippet.el handles the initial configuration)
            ...
            ;; 在格式化完成后自动保存 buffer
            (unless (advice-member-p #'mcg-lsp-format-and-save-after-format
                                     'lsp-bridge-format--update)
