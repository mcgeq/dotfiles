    ...
    (if (not (file-directory-p lsp-bridge-path))
      ...
      (if (require 'lsp-bridge nil t)
          (progn
            ...
            ;; Auto format code: disable for Vue files (volar has performance issues)
            (setq lsp-bridge-enable-auto-format-code t)
            (defun mcg-lsp-auto-format-maybe ()
              "Auto format if not a Vue file."
              (unless (string-suffix-p ".vue" (or (buffer-file-name) ""))
                (lsp-bridge-code-format)))
            (advice-add 'lsp-bridge-auto-format-code :override #'mcg-lsp-auto-format-maybe)
