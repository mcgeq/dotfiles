...
;; Note: auto-mode-alist and major-mode-remap-alist are configured in init-treesit.el
;; Note: Biome formatting functions (mcg-ts-format-buffer, etc.) are in init-lsp.el
...
(defun mcg-ts-setup-keybindings ()
  ...
  ;; Formatting and checking (functions from init-lsp.el)
  (local-set-key (kbd "C-c r f") #'mcg-ts-format-buffer)
  (local-set-key (kbd "C-c b") #'mcg/biome-check)
  (local-set-key (kbd "C-c B") #'mcg/biome-fix)
