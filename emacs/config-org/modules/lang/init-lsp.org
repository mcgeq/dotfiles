...
- Signature help"
  ...
  (with-eval-after-load 'lsp-bridge
    ...
    ;; Python with ty + ruff (use official lsp-bridge config)
    (add-to-list 'lsp-bridge-multi-lang-server-extension-list '(("py") . "ty_ruff"))
    (add-to-list 'lsp-bridge-multi-lang-server-extension-list '(("pyi") . "ty_ruff")))
...
Also organize imports if mode is in `mcg-lsp-organize-imports-modes'."
  (when (and mcg-lsp-format-on-save-enabled
             (bound-and-true-p lsp-bridge-mode))
    ;; First organize imports (for supported modes)
    (when (and mcg-lsp-organize-imports-on-save-enabled
               (member major-mode mcg-lsp-organize-imports-modes))
      (mcg-lsp-organize-imports-silent))
    ;; Then format code
    (when (member major-mode mcg-lsp-format-on-save-modes)
      (lsp-bridge-code-format))))

(defun mcg-lsp-organize-imports-silent ()
  "Organize imports silently (no message output).
Used internally by format-on-save."
  (cond
   ;; Python: Use ruff (process buffer content directly)
   ((member major-mode '(python-mode python-ts-mode))
    (when (and (executable-find "ruff") (buffer-file-name))
      (let ((temp-file (make-temp-file "ruff-isort" nil ".py")))
        (unwind-protect
            (progn
              (write-region (point-min) (point-max) temp-file nil 'silent)
              (when (zerop (call-process "ruff" nil nil nil
                                         "check" "--select" "I" "--fix" temp-file))
   ...
   ;; TypeScript/JavaScript: Use LSP code action
   ((member major-mode '(typescript-mode typescript-ts-mode tsx-ts-mode js-mode js-ts-mode))
