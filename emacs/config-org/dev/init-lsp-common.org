...
(defun mcg-lsp-setup-keybindings ()
  ...
  (local-set-key (kbd "C-c r a") #'lsp-bridge-code-action)  ;; a = action
  (local-set-key (kbd "C-c r f") #'lsp-bridge-code-format)
  (local-set-key (kbd "C-c r i") #'mcg-lsp-organize-imports) ;; i = imports
  ...
  ;; Workspace symbols (避免占用 C-c w 前缀)
