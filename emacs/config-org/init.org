...
(mcg! :ui
      ...
      icons           ; Nerd icons configuration
      ...
      paredit         ; Structural editing (fingertip)
      format          ; Text formatting (wraplish)
      undo            ; Visual undo (vundo)
      multicursor     ; Multi-cursor editing (markmacro)
...
;; Setup idle loading for completion modules
;; These are loaded after Emacs becomes idle
(mcg-add-idle-load-category :completion)

;; Load tools modules immediately
(mcg-load-modules-by-category :tools)

;; Setup idle loading for enhance modules
;; These are loaded after Emacs becomes idle
...
;; Load writing modules immediately
(mcg-load-modules-by-category :writing)
...
;; Setup idle loading (starts loading after idle delay)
...
(add-hook 'emacs-startup-hook
          (lambda ()
            (let ((loaded (length mcg-loaded-modules))
              ...
              (mcg-log "Modules: %d loaded, %d deferred, %d idle-pending, %d errors"
                       loaded deferred idle-pending errors)
