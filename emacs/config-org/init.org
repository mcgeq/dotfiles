...
(mcg-register-loaded-module 'init-modules)    ; 🆕
...
;; 加载UI配置
(require 'init-ui)
;; 使用 awesome-tray，不再使用 doom-modeline
;; (require 'init-doom-modeline)

;; 立即加载 awesome-tray，避免启动时显示 mode-line
(require 'init-awesome-tray)

(mcg-register-loaded-module 'init-ui)
(mcg-register-loaded-module 'init-awesome-tray)
...
(with-fast-gc
  ...
  ;; 🆕 窗口和跳转增强
  (require 'init-ace-window)  ; 快速窗口切换
  (require 'init-avy)         ; 快速跳转
  ...
  (mcg-register-loaded-module 'init-ace-window)  ; 🆕
  (mcg-register-loaded-module 'init-avy)         ; 🆕
...
(defun mcg-load-lang-support ()
  ...
  (mcg-register-loaded-module 'lang-python))     ; 🆕
...
完整文档: org/README-ORG-CONFIG.org"
  ...
  (mcg-org-setup-autoload)  ; 自动加载所有 Org 模块
  
  ;; 显示加载信息
  (message "✓ Org mode 新配置系统已加载")
  (message "  运行 M-x mcg-org-show-status 查看详情")
