...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Disable auto format on save (handled by idle-auto-save instead)
  ;; This prevents double formatting and allows idle-auto-save to control when to format
  ...
  ;; Configure TailwindCSS support for HTML/CSS
...
#+BEGIN_SRC emacs-lisp
  ...
  (lazy-load-set-keys
   '(
     ...
     ("C-c C-u d w" . lsp-bridge-workspace-diagnostic-list) ;; Workspace diagnostics
     )
   lsp-bridge-mode-map)
#+END_SRC
