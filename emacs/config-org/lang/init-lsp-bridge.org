...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Signature help delay (faster response)
  (setq lsp-bridge-signature-help-fetch-idle 0.5)
  ...
  ;; Completion candidate display count
  (setq acm-candidate-match-max-length 60)
  ;; Enable preview when cycling candidates
  ...
  ;; LSP 补全（主要补全源，最激进）
  (setq acm-backend-lsp-candidate-min-length 0)  ; LSP 立即触发，获得最快响应
  ...
  ;; Enable search file words (better contextual completion)
