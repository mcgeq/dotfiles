...
#+BEGIN_SRC emacs-lisp
  ...
  (require 'init-lsp-common)  ;; 通用 LSP 配置和快捷键
  
  ;; Language-specific configurations
  (require 'lang-cpp)
  (require 'lang-rust)
  (require 'lang-python)     ;; Python with ty + ruff
...
** TypeScript SDK Path
#+BEGIN_SRC emacs-lisp
;;; 为 Volar 提供 TypeScript SDK 路径
(defun get-user-tsdk-path ()
  "返回 TypeScript SDK 路径，优先使用项目本地，否则使用全局。"
  (let* ((project-root (when (project-current)
                         (project-root (project-current))))
         (local-tsdk (when project-root
                       (expand-file-name "node_modules/typescript/lib" project-root)))
         (global-tsdk (expand-file-name "~/AppData/Roaming/npm/node_modules/typescript/lib")))
    (cond
     ;; 优先使用项目本地的 TypeScript
     ((and local-tsdk (file-directory-p local-tsdk))
      local-tsdk)
     ;; 否则使用全局安装的 TypeScript
     ((file-directory-p global-tsdk)
      global-tsdk)
     ;; 都没有则返回空字符串
     (t ""))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (with-eval-after-load 'lsp-bridge
    ...
    ;; 选择跳转方式：
    ;; - "vue-full" = TypeScript 跳转（更可靠）
    ;; - "vue-full-volar" = Volar 跳转（需要正确配置 tsdk）
    (add-to-list 'lsp-bridge-multi-lang-server-extension-list '(("vue") . "vue-full"))
    
    ;; Python with ty + ruff (multi-server configuration)
    ;; ty: 类型检查器/LSP (补全、跳转、类型检查)
    ;; ruff: linter/formatter (诊断、格式化、代码修复)
