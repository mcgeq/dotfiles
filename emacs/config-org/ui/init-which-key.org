...
#+BEGIN_SRC emacs-lisp
  ...
  (global-set-key (kbd "C-h w p") 'mcg-which-key-show-all-prefixes)
  ...
  (global-set-key (kbd "C-h w m") 'mcg-which-key-show-major-mode)
#+END_SRC
...
3. MAJOR MODE KEYS
   - C-h w m  Show keys for current major mode

4. ALL PREFIXES
   - C-h w p  Show all defined prefixes
...
")))
  ...
  (global-set-key (kbd "C-h w ?") 'mcg-which-key-tips)
#+END_SRC
