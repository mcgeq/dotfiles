* init-which-key.el
:PROPERTIES:
:HEADER-ARGS: :tangle (concat temporary-file-directory "init-which-key.el") :lexical t
:END:

** Headers

#+BEGIN_SRC emacs-lisp
  ;;; init-which-key.el -- Config for Which Key -*- lexical-binding: t; -*-

  ;; Filename: init-which-key.el
  ;; Description: Config for Which Key - 快捷键提示
  ;; Author: mcge <mcgeq@outlook.com>
  ;; Copyright (C) 2024-2025, mcge, all rights reserved.
  ;; Create   Date: 2025-12-09 20:00:00
  ;; Version: 1.0
  ;; Modified   By: mcgeq <mcgeq@outlook.com>
  ;; Last Modified: <2025-12-09 Mon 20:00>
  ;; Keywords: keybindings, help
  ;; Compatibility: GNU Emacs 31.0.50

  ;;; Commentary:
  ;;
  ;; Which-key 显示当前可用的快捷键
  ;; 当你按下一个前缀键后，会弹出一个窗口显示所有可用的后续键
  ;;

#+END_SRC

** Require
#+BEGIN_SRC emacs-lisp
;;; Require:
(require 'which-key)

#+END_SRC

** Configuration
#+BEGIN_SRC emacs-lisp
;;; Code:

;; 启用 which-key
(add-hook 'after-init-hook #'which-key-mode)

;; ═══════════════════════════════════════════════════════════
;; 基础设置
;; ═══════════════════════════════════════════════════════════

;; 显示延迟 (秒)
(setq which-key-idle-delay 0.3)

;; 次要延迟 (连续按键时的延迟)
(setq which-key-idle-secondary-delay 0.05)

;; 弹出窗口位置: bottom, top, left, right, frame
(setq which-key-popup-type 'side-window)
(setq which-key-side-window-location 'bottom)

;; 窗口最大高度 (行数比例)
(setq which-key-side-window-max-height 0.3)

;; 窗口最大宽度
(setq which-key-side-window-max-width 0.4)

;; 窗口最小高度
(setq which-key-min-display-lines 6)

;; ═══════════════════════════════════════════════════════════
;; 外观美化
;; ═══════════════════════════════════════════════════════════

;; 排序方式: 按键名字母排序
(setq which-key-sort-order 'which-key-key-order-alpha)

;; 允许分页显示更多内容
(setq which-key-allow-multiple-replacements t)
(setq which-key-allow-imprecise-window-fit nil)

;; 显示前缀描述位置
(setq which-key-show-prefix 'left)

;; 美化分隔符 (使用箭头)
(setq which-key-separator "  ")

;; 特殊键名替换 (更易读的图标/符号)
(setq which-key-special-keys '("SPC" "TAB" "RET" "DEL" "ESC"))

;; 键名美化替换
(setq which-key-key-replacement-alist
      '(("<left>"  . "←")
        ("<right>" . "→")
        ("<up>"    . "↑")
        ("<down>"  . "↓")
        ("<return>" . "⏎")
        ("<tab>"   . "⇥")
        ("<escape>" . "⎋")
        ("<backspace>" . "⌫")
        ("<delete>" . "⌦")
        ("SPC"     . "␣")
        ("RET"     . "⏎")
        ("TAB"     . "⇥")
        ("DEL"     . "⌫")))

;; 添加图标到命令描述 (需要 nerd-icons 或 all-the-icons)
(setq which-key-add-column-padding 2)
(setq which-key-max-description-length 32)
(setq which-key-ellipsis "..")

;; Unicode 美化边框
(setq which-key-unicode-correction 3)

;; ═══════════════════════════════════════════════════════════
;; 自定义外观 (Face)
;; ═══════════════════════════════════════════════════════════

;; 按键样式
(custom-set-faces
 '(which-key-key-face
   ((t (:foreground "#7aa2f7" :weight bold))))  ; 蓝色粗体
 '(which-key-separator-face
   ((t (:foreground "#565f89"))))               ; 灰色分隔符
 '(which-key-note-face
   ((t (:foreground "#9ece6a"))))               ; 绿色注释
 '(which-key-command-description-face
   ((t (:foreground "#c0caf5"))))               ; 浅色命令描述
 '(which-key-group-description-face
   ((t (:foreground "#bb9af7" :weight bold))))) ; 紫色组描述

#+END_SRC

** Prefix Descriptions
#+BEGIN_SRC emacs-lisp
;;; 前缀描述 (带图标)

;; 为常用前缀添加描述 (使用 emoji/符号美化)
(which-key-add-key-based-replacements
  ;; C-c 前缀组
  "C-c"     "⚙ User"
  "C-c g"   " Git"
  "C-c g b" "  Branch"
  "C-c g r" "  Remote"
  "C-c g m" "  Submodule"
  "C-c g F" "  File"
  "C-c m"   "✎ Markmacro"
  "C-c n"   "📝 Notes/Org"
  "C-c r"   "🔧 Refactor"
  "C-c s"   "🔍 Search"
  "C-c t"   "📑 Tabs"
  "C-c u"   "↩ Undo"
  "C-c y"   "✂ Snippet"
  "C-c f"   "📁 Files"
  "C-c w"   "🪟 Window"
  "C-c w w" "  Switch"
  "C-c w d" "  Delete"
  "C-c w s" "  Swap"
  "C-c w m" "  Move"
  "C-c w o" "  Only (delete others)"
  "C-c w v" "  Split Vertical"
  "C-c w h" "  Split Horizontal"
  
  ;; 标准前缀
  "C-x"     "📋 Emacs"
  "C-x r"   "📌 Register"
  "C-x p"   "📦 Project"
  "C-x 4"   "🪟 Other Window"
  "C-x 5"   "🖼 Frame"
  "C-h"     "❓ Help"
  "M-g"     "➡ Goto"
  "M-g w"   "  Word (avy)"
  "M-g g"   "  Line (avy)"
  "M-g c"   "  Char timer (avy)"
  "M-g j"   "  Char in line (avy)"
  "M-g y"   "  Copy line (avy)"
  "M-g m"   "  Move line (avy)"
  "M-g k"   "  Kill line (avy)"
  "M-s"     "🔎 Search")

#+END_SRC

** Performance
#+BEGIN_SRC emacs-lisp
;;; 性能优化

;; 禁用某些模式下的 which-key (如 minibuffer)
(setq which-key-allow-evil-operators nil)
(setq which-key-show-remaining-keys nil)

;; 减少更新频率
(setq which-key-compute-remaps nil)

;; 修复 C-g 取消后 which-key 窗口不关闭的问题
(defun mcg/which-key-abort-and-hide ()
  "取消命令并隐藏 which-key 窗口。"
  (interactive)
  (which-key--hide-popup)
  (keyboard-quit))

;; 在 which-key 显示时，C-g 同时关闭 which-key 窗口
(advice-add 'keyboard-quit :before
            (lambda (&rest _)
              (when (which-key--popup-showing-p)
                (which-key--hide-popup))))

#+END_SRC

** Ends
#+BEGIN_SRC emacs-lisp
(provide 'init-which-key)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; init-which-key.el ends here
#+END_SRC
