...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Last Modified:  <2025-12-03 Wed 19:40>
...
Note: If lsp-bridge-enable-auto-format-code is t, formatting will happen automatically
via before-save-hook, so this option can be set to nil to avoid double formatting."
    :type 'boolean
    :group 'idle-auto-save)
  ...
  (defun idle-auto-save-save-buffer ()
    "Save current buffer.
Formatting is handled by idle-auto-save-format-buffer in before-save-hook."
    (let ((write-region-inhibit-fsync t))
      ;; Save buffer (before-save-hook will handle formatting)
  ...
  (defun idle-auto-save-format-buffer ()
    "Format buffer with lsp-bridge if enabled.
Waits for async formatting to complete."
    (when (and idle-auto-save-format-before-save
      ...
      (ignore-errors
        ...
        ;; Wait for formatting to complete (max 1 second)
        (let ((max-wait 1.0)
              (wait-interval 0.05)
              (elapsed 0)
              (initial-tick (buffer-chars-modified-tick)))
          ;; Wait for buffer modification (formatting changes it)
          (while (and (< elapsed max-wait)
                      (= (buffer-chars-modified-tick) initial-tick))
            (sleep-for wait-interval)
            (setq elapsed (+ elapsed wait-interval)))
          ;; Additional small wait to ensure formatting is complete
          (sleep-for 0.1)))))
  ...
  (defun idle-auto-save-enable ()
    ...
    (add-hook 'before-save-hook 'idle-auto-save-format-buffer)
  ...
  (defun idle-auto-save-disable ()
    ...
    (interactive)
    (idle-auto-save-cancel-timer)
    (remove-hook 'before-save-hook 'idle-auto-save-format-buffer)
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Enable format before auto-save
  ;; Waits for lsp-bridge async formatting to complete before saving
