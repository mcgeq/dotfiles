...
via before-save-hook, so this option can be set to nil to avoid double formatting."
  ...
  (defun idle-auto-save-save-buffer ()
    "Save current buffer.
Formatting is handled by lsp-bridge via before-save-hook if enabled."
    (let ((write-region-inhibit-fsync t))
      ;; Optional: Format before save if explicitly enabled
      ;; (Usually not needed as lsp-bridge handles it via before-save-hook)
      (when idle-auto-save-format-before-save
        (ignore-errors
          (when (and (fboundp 'lsp-bridge-code-action-format-code)
                     (boundp 'lsp-bridge-mode)
                     lsp-bridge-mode)
            (lsp-bridge-code-action-format-code))))
      
      ;; Save buffer (will trigger before-save-hook including lsp-bridge formatting)
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Disable format before auto-save (lsp-bridge handles formatting via before-save-hook)
  ;; This ensures both auto-save and manual save use the same formatting mechanism
  (setq idle-auto-save-format-before-save nil)
