...
if cfg.enable_imselect then
  autocmd("InsertLeave", {
    group = myAutoGroup,
    callback = require("pisces.utils.im-select").insertLeave,
  })

  autocmd("InsertEnter", {
    group = myAutoGroup,
    callback = require("pisces.utils.im-select").insertEnter,
  })
end
...
autocmd("BufWritePre", {
  group = myAutoGroup,
  pattern = require("pisces.env").getFormatOnSavePattern(),
  callback = function()
    vim.lsp.buf.format()
  end,
})
