...
local cfg = pRequire("pisces").config.cmp

local pathUtils = require("pisces.utils.path")
...
pRequire("luasnip.loaders.from_lua").load({
  paths = pathUtils.join(pathUtils.getConfig(), "lua", "pisces", "cmp", "snippets", "lua"),
})
pRequire("luasnip.loaders.from_vscode").lazy_load({
  paths = pathUtils.join(pathUtils.getConfig(), "lua", "pisces", "cmp", "snippets", "vscode"),
})
