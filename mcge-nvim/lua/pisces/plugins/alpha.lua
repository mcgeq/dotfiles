...
if cfg and cfg.enable then
	...
	local function mru(start, cwd, items_number, opts)
		...
		for _, v in pairs(vim.v.oldfiles) do
			...
			local cwd_cond = cwd and vim.startswith(v, cwd) or true
	...
	local buttons = {
		...
		val = {
			...
			dashboard.button(cfg.keys.find_files, "󰈞  Find file"),
			dashboard.button(cfg.keys.live_grep, "󰮗  Live grep"),
	...
	local versionSection = {
		type = "text",
		val = require("pisces").version .. " make with ♥ by mcge",
		opts = {
			position = "center",
			hl = "Type",
			-- wrap = "overflow";
