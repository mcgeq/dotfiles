...
local init = function(userConfig)
  table.insert(enabledEnv, require("pisces.env.common")(userConfig))
  if userConfig.lua.enable then
    table.insert(enabledEnv, require("pisces.env.lua")(userConfig.lua))
  end
  if userConfig.frontend.enable then
    table.insert(enabledEnv, require("pisces.env.frontend")(userConfig.frontend))
  end
  if userConfig.golang.enable then
    table.insert(enabledEnv, require("pisces.env.golang")(userConfig.golang))
  end
  if userConfig.clangd.enable then
    table.insert(enabledEnv, require("pisces.env.clangd")(userConfig.clangd))
  end
  if userConfig.rust.enable then
    table.insert(enabledEnv, require("pisces.env.rust")(userConfig.rust))
  end
  if userConfig.bash.enable then
    table.insert(enabledEnv, require("pisces.env.bash")(userConfig.bash))
  end
  if userConfig.python.enable then
    table.insert(enabledEnv, require("pisces.env.python")(userConfig.python))
  end
  if userConfig.ruby.enable then
    table.insert(enabledEnv, require("pisces.env.ruby")(userConfig.ruby))
  end
  if userConfig.json.enable then
    table.insert(enabledEnv, require("pisces.env.json")(userConfig.json))
  end
  if userConfig.yaml.enable then
    table.insert(enabledEnv, require("pisces.env.yaml")(userConfig.yaml))
  end
  if userConfig.markdown.enable then
    table.insert(enabledEnv, require("pisces.env.markdown")(userConfig.markdown))
  end
  if userConfig.docker.enable then
    table.insert(enabledEnv, require("pisces.env.docker")(userConfig.docker))
  end
  if userConfig.solidity.enable then
    table.insert(enabledEnv, require("pisces.env.solidity")(userConfig.solidity))
  end
  if userConfig.java.enable then
    table.insert(enabledEnv, require("pisces.env.java")(userConfig.java))
  end
end
