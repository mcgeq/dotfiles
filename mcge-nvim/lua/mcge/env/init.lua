...
local init = function(userConfig)
  table.insert(enabledEnv, require("mcge.env.common")(userConfig))
  if userConfig.lua.enable then
    table.insert(enabledEnv, require("mcge.env.lua")(userConfig.lua))
  end
  if userConfig.frontend.enable then
    table.insert(enabledEnv, require("mcge.env.frontend")(userConfig.frontend))
  end
  if userConfig.golang.enable then
    table.insert(enabledEnv, require("mcge.env.golang")(userConfig.golang))
  end
  if userConfig.clangd.enable then
    table.insert(enabledEnv, require("mcge.env.clangd")(userConfig.clangd))
  end
  if userConfig.rust.enable then
    table.insert(enabledEnv, require("mcge.env.rust")(userConfig.rust))
  end
  if userConfig.bash.enable then
    table.insert(enabledEnv, require("mcge.env.bash")(userConfig.bash))
  end
  if userConfig.python.enable then
    table.insert(enabledEnv, require("mcge.env.python")(userConfig.python))
  end
  if userConfig.ruby.enable then
    table.insert(enabledEnv, require("mcge.env.ruby")(userConfig.ruby))
  end
  if userConfig.json.enable then
    table.insert(enabledEnv, require("mcge.env.json")(userConfig.json))
  end
  if userConfig.yaml.enable then
    table.insert(enabledEnv, require("mcge.env.yaml")(userConfig.yaml))
  end
  if userConfig.markdown.enable then
    table.insert(enabledEnv, require("mcge.env.markdown")(userConfig.markdown))
  end
  if userConfig.docker.enable then
    table.insert(enabledEnv, require("mcge.env.docker")(userConfig.docker))
  end
  if userConfig.solidity.enable then
    table.insert(enabledEnv, require("mcge.env.solidity")(userConfig.solidity))
  end
  if userConfig.java.enable then
    table.insert(enabledEnv, require("mcge.env.java")(userConfig.java))
  end
end
