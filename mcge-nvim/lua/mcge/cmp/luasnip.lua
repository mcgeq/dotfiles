...
local cfg = pRequire("mcge").config.cmp

local pathUtils = require("mcge.utils.path")
...
pRequire("luasnip.loaders.from_lua").load({
  paths = pathUtils.join(pathUtils.getConfig(), "lua", "mcge", "cmp", "snippets", "lua"),
})
pRequire("luasnip.loaders.from_vscode").lazy_load({
  paths = pathUtils.join(pathUtils.getConfig(), "lua", "mcge", "cmp", "snippets", "vscode"),
})
