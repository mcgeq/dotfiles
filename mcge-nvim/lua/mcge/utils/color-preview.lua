...
local function next_color(prompt_buffer)
  ...
  local selected = action_state.get_selected_entry()
  require("mcge").config.colorscheme = selected[1]
  package.loaded["mage.plugins.lualine"] = nil
...
local function prev_color(prompt_buffer)
  ...
  local selected = action_state.get_selected_entry()
  require("mcge").config.colorscheme = selected[1]
  package.loaded["mcge.plugins.lualine"] = nil
  require("mcge.plugins.lualine")
  local cmd = "colorscheme " .. selected[1]
  vim.cmd(cmd)
end
...
local opts = {
  ...
  initial_mode = "normal",
  finder = finders.new_table(mgcolors),
