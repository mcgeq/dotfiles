...
- **项目根识别** - 统一识别 Git、Python、Node、Rust、Go、Zig、CMake 等项目根目录
- **工作区会话** - 基于 Startify 按项目根保存和加载会话
- **上下文终端** - 用 Floaterm 在 cwd、项目根或当前文件目录打开终端
- **统一快捷键** - 提供独立的 `<leader>P*`、`<leader>m*`、`<leader>l*` 命名空间

### ▶ 运行与测试流

- **统一命令** - `:RunnerInfo`、`:RunnerBuild`、`:RunnerRun`、`:RunnerTest`、`:RunnerTestFile`、`:RunnerRepeat`、`:RunnerClean`
- **统一快捷键** - 全局使用 `<leader>r*` 命名空间，不再把运行键分散在各个 filetype 局部映射里
- **Python 默认策略** - 优先使用 `uv + ty`，缺失时再回退到 `uv + python` 或本地/系统 Python
- **前端脚本复用** - 自动识别 pnpm、bun、yarn、npm，并复用 `package.json` 中的脚本
...
│   ├── module.vim              # 共享模块辅助
│   ├── runner.vim              # 统一 build/run/test 辅助
│   ├── workspace.vim           # 项目根、会话、终端辅助
│   └── health.vim              # 健康检查系统
├── modules/                    # 功能模块
│   ├── editor/                 # 编辑器增强
...
│   │   ├── runner.vim          # 共享运行/测试快捷键
│   │   ├── terminal.vim        # Floaterm 快捷键
│   │   ├── workspace.vim       # 项目/会话/终端上下文快捷键
...
- 运行与测试：`<leader>ri`、`<leader>rb`、`<leader>rr`、`<leader>rt`、`<leader>rT`
- 工作区与会话：`<leader>Pi`、`<leader>Pr`、`<leader>ms`、`<leader>ml`
- LSP 与代码动作：`gd`、`gr`、`<leader>rn`、`<leader>af`、`<leader>aa`
- 浏览器与大纲：`<leader>ee`、`<leader>vv`、`<leader>vf`
- 窗口与缓冲区：`<C-h/j/l>`、`<C-tab>`、`<C-x><C-s>`
- 终端：`<F7>`、`<F8>`、`<F9>`、`<F12>`、`<leader>lt`、`<leader>lp`、`<leader>lb`
- 界面与配置：`<leader>`、`<leader>ur`
