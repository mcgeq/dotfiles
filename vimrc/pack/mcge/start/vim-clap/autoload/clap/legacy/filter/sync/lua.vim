...
if has('nvim-0.5')
  ...
  function! clap#legacy#filter#sync#lua#(query, candidates, _winwidth, enable_icon, match_scope) abort
    let g:_clap_lua_query = a:query
    let g:_clap_lua_candidates = a:candidates
    let g:_clap_lua_enable_icon = a:enable_icon
    let g:_clap_lua_match_scope = a:match_scope

lua << EOF
local fzy_filter = require('fzy_filter')
vim.g.__clap_fuzzy_matched_indices, vim.g._clap_lua_filtered =
    fzy_filter.do_fuzzy_match(vim.g._clap_lua_query, vim.g._clap_lua_candidates, vim.g._clap_lua_enable_icon, vim.g._clap_lua_match_scope)
EOF
...
matched_indices, _clap_lua_filtered =
    fzy_filter.do_fuzzy_match(vim.eval('a:query'), lines, vim.eval('a:enable_icon'), vim.eval('a:match_scope'))
