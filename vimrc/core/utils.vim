...
# 对外暴露可比较路径，供跨模块路径判断与工作区逻辑复用
def g:NormalizeComparablePath(path: string): string
  return NormalizeComparablePath(path)
enddef

# 为 Floaterm 这样的 Ex 命令参数做转义，避免路径中的空格和反斜杠被拆开
def g:EscapeFloatermArg(value: string): string
  var escaped = substitute(value, '\\', '\\\\', 'g')
