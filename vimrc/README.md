...
- **Project Root Detection** - Shared root detection for Git, Python, Node, Rust, Go, Zig, and CMake projects
- **Workspace Sessions** - Startify-backed session save/load per detected project root
- **Context Terminal** - Open Floaterm in cwd, project root, or current buffer directory
- **Unified Keymaps** - Dedicated `<leader>P*`, `<leader>m*`, and `<leader>l*` namespaces

### ▶ Runner Flow

- **Unified Commands** - `:RunnerInfo`, `:RunnerBuild`, `:RunnerRun`, `:RunnerTest`, `:RunnerTestFile`, `:RunnerRepeat`, `:RunnerClean`
- **Shared Keymaps** - Global `<leader>r*` runner namespace instead of scattered per-filetype mappings
- **Python Default** - Prefer `uv + ty`; fall back to `uv + python` or local/system Python when needed
- **Frontend Scripts** - Reuse `package.json` scripts with package-manager detection for pnpm, bun, yarn, and npm
...
│   ├── runner.vim              # Unified build/run/test helpers
│   ├── workspace.vim           # Project root, session, terminal helpers
...
│   │   ├── runner.vim          # Shared build/run/test mappings
│   │   ├── terminal.vim        # Floaterm mappings
│   │   ├── workspace.vim       # Project/session/terminal context mappings
...
- Runner: `<leader>ri`, `<leader>rb`, `<leader>rr`, `<leader>rt`, `<leader>rT`
- Workspace and session: `<leader>Pi`, `<leader>Pr`, `<leader>ms`, `<leader>ml`
- LSP and actions: `gd`, `gr`, `<leader>rn`, `<leader>af`, `<leader>aa`
- Explorer and outline: `<leader>ee`, `<leader>vv`, `<leader>vf`
- Windows and buffers: `<C-h/j/l>`, `<C-tab>`, `<C-x><C-s>`
- Terminal: `<F7>`, `<F8>`, `<F9>`, `<F12>`, `<leader>lt`, `<leader>lp`, `<leader>lb`
- UI and config: `<leader>`, `<leader>ur`
